...
public class TestRequestAttributes {
  ...
  @Test
  public void testAsyncRequestAttributesFactoryOverridesStaticAttributes()
    throws IOException, ExecutionException, InterruptedException {
    Configuration conf = TEST_UTIL.getConfiguration();
    try (AsyncConnection conn = ConnectionFactory.createAsyncConnection(conf).get()) {
  ...
  @Test
  public void testAsyncRequestAttributesFactoryWithThreadLocal()
    throws IOException, ExecutionException, InterruptedException {
    Configuration conf = TEST_UTIL.getConfiguration();
    ThreadLocal<Map<String, byte[]>> threadLocalAttributes = new ThreadLocal<>();
    try (AsyncConnection conn = ConnectionFactory.createAsyncConnection(conf).get()) {
      AsyncTable<?> table = conn.getTableBuilder(TABLE_NAME).setRequestAttributesFactory(() -> {
        Map<String, byte[]> attrs = threadLocalAttributes.get();
        return attrs != null ? attrs : Collections.emptyMap();
      }).build();

      Map<String, byte[]> attributes = new HashMap<>();
      attributes.put(FACTORY_KEY, FACTORY_VALUE);
      threadLocalAttributes.set(attributes);
      table.get(new Get(ROW_KEY_FACTORY_GET)).get();

      threadLocalAttributes.set(new HashMap<>());
      table.get(new Get(ROW_KEY8)).get();
    } finally {

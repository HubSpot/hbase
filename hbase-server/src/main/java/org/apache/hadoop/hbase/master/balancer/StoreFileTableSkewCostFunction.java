 ...
 * storefile sizes (in MB) per table using the CostFromRegionLoadFunction framework.
 * For each table, it computes a per-server aggregated storefile size by summing
 * the average storefile size for each region (if there are multiple load metrics,
 * it averages them). The imbalance cost (as computed by DoubleArrayCost) is then
 * used to drive the balancer to reduce differences between servers.
 */
...
public class StoreFileTableSkewCostFunction extends CostFromRegionLoadFunction {
  ...
  public StoreFileTableSkewCostFunction(Configuration conf) {
    this.setMultiplier(conf.getFloat(STOREFILE_TABLE_SKEW_COST_KEY, DEFAULT_STOREFILE_TABLE_SKEW_COST));
  }
        ...
        for (int server = 0; server < cluster.numServers; server++) {
          ...
          for (int region : cluster.regionsPerServer[server]) {
            if (cluster.regionIndexToTableIndex[region] == tableIndex) {
              Collection<BalancerRegionLoad> loads = cluster.regionLoads[region];
              double regionCost = 0;
    ...
    if (newServer >= 0) {
      ...
      for (int r : cluster.regionsPerServer[newServer]) {
        if (cluster.regionIndexToTableIndex[r] == tableIdx) {
          Collection<BalancerRegionLoad> loads = cluster.regionLoads[r];
          double regionCost = 0;
          if (loads != null && !loads.isEmpty()) {
            for (BalancerRegionLoad rl : loads) {
              regionCost += getCostFromRl(rl);

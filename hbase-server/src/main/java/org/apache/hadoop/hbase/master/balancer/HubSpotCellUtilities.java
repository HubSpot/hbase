...
public final class HubSpotCellUtilities {
  ...
  public static final Gson OBJECT_MAPPER = new GsonBuilder()
    ...
    .registerTypeAdapter(
      ...
      (JsonDeserializer) (json, typeOfT, context) -> {
        ...
        RegionInfo result = RegionInfoBuilder
          .newBuilder(tb)
          .setSplit(split)
          .setRegionId(regionId)
          .setReplicaId(replicaId)
          .setStartKey(startKeyBytes)
          .setEndKey(endKeyBytes)
          .build();
        return result;
      }
    )
    .addDeserializationExclusionStrategy(
      new ExclusionStrategy() {
        ...
        public boolean shouldSkipField(FieldAttributes f) {
          return (
            f.getName().equals("serversToIndex") ||
  ...
  public static Set<Short> toCells(byte[] rawStart, byte[] rawStop, short numCells) {
    return range(
      padToTwoBytes(rawStart, (byte) 0),
      padToTwoBytes(rawStop, (byte) -1),
      numCells
    );
  }
  ...
  static class Int2IntCounterMapAdapter
    ...
    @Override
    public Int2IntCounterMap deserialize(
      JsonElement json,
      Type typeOfT,
      ...
      try {
        resizeThresholdField =
          Int2IntCounterMap.class.getDeclaredField("resizeThreshold");
        entryField = Int2IntCounterMap.class.getDeclaredField("entries");
